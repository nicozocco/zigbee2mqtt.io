...
You can calibrate your device (to enable position support) by closing the shutter (or opening it if your cover position is inverted) and pressing both the up and down buttons for about 5 seconds until you see a blinking yellow light. Follow the steps depending on the calibration mode:
* NLLV modes:
When switching to those modes, the switch starts the calibration itself. In case the blind is in an intermediate position, it will be raised prior starting the calibration cycle. The switch will then command the blind up and down twice fully automatically. The entire process can last up to 5 minutes.
* Up / Down / Stop:
